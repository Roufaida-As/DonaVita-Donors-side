      ...
      builder: (context, AsyncSnapshot<QuerySnapshot> snapshot) {
        ...
        if (snapshot.hasError) {
          Dialogs.showSnackBar(
              "Error", "Error fetching Favorites: ${snapshot.error}", true);
          return const Center(
  ...
  Widget build(BuildContext context) {
    return StreamBuilder(
      ...
      builder: (context, AsyncSnapshot<DocumentSnapshot> snapshot) {
        ...
        if (snapshot.hasError) {
          Dialogs.showSnackBar(
              "Error", "Error fetching annonces: ${snapshot.error}", true);
          return const Center(
            child: Text('Error fetching data'),
          );
        }
        ...
        final orgLogoUrl = data['organizationLogoUrl'] as String? ??
            'https://example.com/default_logo.png';
        final description =
            data['description'] as String? ?? 'Description Not Available';
...
class _FavoriteItemState extends State<FavoriteItem> {
  ...
  Future<void> deleteFavorite(String orgId) async {
    try {
      ...
      Dialogs.showSnackBar("Success", "Document deleted successfully", false);
    } catch (error) {
      Dialogs.showSnackBar("Error", "Error deleting document: $error", true);
    }
  }
}
