...
class AnnouncementItem extends StatefulWidget {
  ...
  const AnnouncementItem(
      {super.key, required this.announcement, required this.isFirst});
      ...
      child: ListTile(
        ...
        leading: Stack(
          children: [
            CircleAvatar(
              backgroundImage:
                  NetworkImage(widget.announcement.organizationLogoUrl),
            ),
            if (!_isClicked)
              Positioned(
                top: 0,
  ...
  Future<void> _addToFavorites(Announcement announcement) async {
    try {
      ...
      Dialogs.showSnackBar(
          'Success', 'Announcement added to favorites successfully', false);
    } catch (error) {
      Dialogs.showSnackBar(
          'Error', 'Error adding announcement to favorites: $error', true);
    }
  }
}
